package mwx.zhouxin.com.videoplayer.full.full.part;
 ...
 */
/*
* 一个自定义的VdeoView, 使用MdidaPlayer+surfaceview
* mediaplayer来做视频播放的控制，surfaceView 来显示视频
* 实现简单的播放控制：在视频上放一个播放暂停按钮，一个进度条和一个全屏按钮
*实现思路状态:
* init初始化方法 ：初始化自定义视图
* 提供 setVideoPath方法（一定要在onResume方法调用前来调用）:设置播放 谁
* 提供onResume方法 进入时调用 :初始化MediaPlayer，准备mediaplayer
* onPause 出去时调用:释放MediaPlayer,终止mediaplayer
*
* */
...
public class SimplerVideoView extends FrameLayout {
    public SimplerVideoView(Context context) {
        this(context, null);
    }

    public SimplerVideoView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SimplerVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
